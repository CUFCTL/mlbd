...
Note that we may not always need to request GPUs in our sessions, and we should avoid requesting them when we don't. You do need to request one for this one-off environment setup process.
...
This command will ask you to confirm the installation, and then it will take a while to install everything. If everything has been done properly until now, the command will install packages and libraries, including PyTorch and TensorFlow. Once it finishes, run this command to enable use of the created environment in Jupyter notebooks.
...
### Quick test for PyTorch and TensorFlow

With the environment activated, and a GPU available in the session, run the following lines in the terminal, one after the other.
```bash
python -c "import torch; print(torch.__version__, torch.version.cuda, torch.cuda.is_available())"
python -c "import tensorflow as tf; print(tf.__version__); print(tf.config.list_physical_devices('GPU'))"
```
They should both print out some information without the word "error" in the output.

### Create a Jupyter kernel for your environment

Now, click the "+" icon again and select the notebook option called "Python 3 (dlbd)". Creating a notebook with this kernel lets you use any Python packages installed in your Anaconda environment.
